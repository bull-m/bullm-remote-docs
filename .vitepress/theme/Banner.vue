...
watch(banner, (newBanner) => {
  if (newBanner) {
    // 检查是否应该显示
    open.value = checkShouldShowBanner()
  } else {
    open.value = false
  }
}, {immediate: true})
...
<template>
  <div class="banner" v-if="open && banner">

    <div class="banner-text">
      <p class="banner-title">{{ banner.title }}</p>
      <a
          :target="banner.blank ? '_blank' : ''"
...
.glow.glow--blue {
  ...
  pointer-events: none;
  border-radius: 100%;
  background: linear-gradient(180deg, #61ff8a, #0000);
  filter: blur(15vw);
  transform: none;
  opacity: 0.3;
}
...
.primary-action {
  display: inline-block;
  background: radial-gradient(140.35% 140.35% at 175% 94.74%, #2bfd6b, #34fe6100),
  radial-gradient(89.94% 89.94% at 18.42% 15.79%, #4df07a, #41ff8a00);
  color: #fff;
  padding: 6px 12px;
  border-radius: 6px;
  font-size: 16px;
